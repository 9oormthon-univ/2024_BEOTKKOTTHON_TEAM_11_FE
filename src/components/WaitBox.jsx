...
const BabDiv = styled.div`
    ...
    transition:
        ...
        color 0.3s; // 부드러운 색상 전환을 위한 transition 추가
`;
...
function WaitBox(props) {
    const { id, name, userRole, location, confirmCount, allResponded, state } =
        props.event;
            ...
            <PeopleArrowContainer>
                <PeopleContainer>
                    ...
                    <AnswerPeople hover={hover}>
                        {allResponded
                            ? '모두 응답 완료했어요!'
                            : `현재 ${confirmCount}명이 응답 완료했어요!`}
                    </AnswerPeople>
                </PeopleContainer>
                <Button
                    onClick={() => navigate(`/event/${id}/pending`)}
                    hover={hover}
                >
                    <IoIosArrowForward />
                </Button>
